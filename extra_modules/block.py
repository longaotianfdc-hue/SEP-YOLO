...
class CARAFE(nn.Module):
    def __init__(self, c, k_enc=3, k_up=5, c_mid=64, scale=2,dilation=2):
        ...
        # 2. 核心改进: 增大编码器的感受野
        # 使用 nn.Conv2d 实现空洞卷积
        enc_padding = (k_enc // 2) * dilation  # 保持输出尺寸不变所需的 padding
        self.enc = nn.Conv2d(c_mid, (scale * k_up) ** 2,
                             kernel_size=k_enc,
                             padding=enc_padding,
                             dilation=dilation,  # 核心改变：增加空洞率
                             bias=False)
        self.bn_enc = nn.BatchNorm2d((scale * k_up) ** 2)  # 原始 Conv 模块中的 BN 移到这里
    ...
    def forward(self, X):
        ...
        W = self.bn_enc(self.enc(W))  # b * (s*k_up)^2 * h * w
