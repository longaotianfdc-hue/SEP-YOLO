...
import torch.nn as nn
import torch.nn.functional as F
from torch.fft import fft,rfft2,irfft2

class FFEMS(nn.Module):
    """
    YOLO频域增强模块 - 在保留空间特征的同时增强高频边界信息

    设计特点:
    1. 多尺度频域处理：自适应捕捉不同尺度的边界特征
    2. 空间特征保留：通过残差连接确保原始空间信息不丢失
    3. 注意力引导：通道+空间双重注意力机制聚焦关键区域
    4. 归一化稳定：LayerNorm确保训练稳定性

    参数:
    - in_channels: 输入通道数（必须与YOLO Block输入一致）
    - reduction: 通道压缩比例（默认16）
    - num_scales: 多尺度高频保留的数量（默认3）
    - cutoff_ratios: 初始截止频率比例列表（可选）
    """

    def __init__(self, in_channels, reduction=16, num_scales=3, cutoff_ratios=None):
        super().__init__()
        ...
        # -------------------- 多尺度频域滤波器 --------------------
        default_cutoffs = cutoff_ratios or [0.2 * (i + 1) for i in range(num_scales)]
        self.cutoff_params = nn.ParameterList([
            nn.Parameter(torch.tensor(ratio), requires_grad=True)
            for ratio in default_cutoffs
        ])
        ...
        # 关键修复：在__init__中定义LayerNorm，确保设备一致性
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # ==================== 2. 多尺度频域处理 ====================
        multi_scale_outputs = []
        freq_domain = rfft2(x, norm='ortho')
        ...
        for i in range(self.num_scales):
            ...
            spatial_output = irfft2(filtered_freq, s=(H, W), norm='ortho')

            # 关键修复：使用预定义的LayerNorm层
            spatial_output = spatial_output.permute(0, 2, 3, 1)  # [B, H, W, C]
            spatial_output = self.layer_norm(spatial_output)
            spatial_output = spatial_output.permute(0, 3, 1, 2)  # [B, C, H, W]
        ...
        # ==================== 4. 特征融合 ====================
        enhanced_feat = spatial_feat * freq_attn
        combined = torch.cat([enhanced_feat, spatial_feat], dim=1)
...
class C3k_FFEM(C3k):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5, k=3):
        ...
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(FFEMS(c_) for _ in range(n)))

class FFEM(C3k2):
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        super().__init__(c1, c2, n, c3k, e, g, shortcut)
        self.m = nn.ModuleList(C3k_FFEM(self.c, self.c, 2, shortcut, g) if c3k else FFEMS(self.c) for _ in range(n))


class EMFAB(nn.Module):
    def __init__(self, c1, c2):
        super().__init__()
        # 通道调整
        ...
        # 特征融合
        self.fuse_conv = nn.Conv2d(4 * c2, c2, kernel_size=1)
    ...
    def forward(self, x):
        ...
        # 拼接特征
        fused = torch.cat([b3, b5, b7, pool], dim=1)
